...
export async function getUserWeekCalendar(
  ...
  for (let i = 0; i < 13; i++) {
    ...
    var response: PageCollection = await graphClient!
      ...
      .query({
        startDateTime: 2013 + i + "-01-01T00:00:00-08:00",
        endDateTime: 2014 + i + "-01-01T00:00:00-08:00",
      })
      .select("subject,organizer,start,end")
      .orderby("start/dateTime")
      .top(500)
...
export async function createMessage(
  ...
  // JSON representation of the new event is sent in the
  // request body
  return await graphClient!.api("/me/messages").post(newMessage);
}
// </CreateMessageSnippet>
...
export async function replyMessage(
  ...
  var res = await graphClient!
    .api("me/messages/" + mailId + "/reply")
    .post(newReplyMessage);
...
export async function forwardMessage(
  ...
  ensureClient(authProvider);
  await graphClient!
    .api("me/messages/" + mailId + "/forward")
    .post(newReplyMessage);
}
// </ReplyMessageSnippet>
...
export async function getMessageAttachments(
  ...
  var response = await graphClient!
    .api(
      "/me/mailFolders/" + mailFolder + "/messages/" + mailId + "/attachments"
    )
    .get();
  //  return response.value;
